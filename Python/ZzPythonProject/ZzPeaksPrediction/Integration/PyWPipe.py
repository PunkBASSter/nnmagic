...
class ServerClient:
    def __init__(self, handle, mode, maxmessagesz):
        ...
        '''
            The `rlock` prevents you from issuing a write
    ...
    def read(self):
        ...
        if not self.alive and not self.can_read():
...
class Server(Base):
    def __init__(self, name, mode, *, max_clients = 2, max_message_size = 4096, max_time = 100):
        self.name = name
        self.mode = mode
        self.clients = []
        self._max_clients = max_clients
        self._max_message_size = 4096
        self.maxtime = max_time
        self.shutdown = False
        self.t = threading.Thread(target = self.server_entry)
    ...
    def wait_for_data(self, timeout = None, interval = 0.01):
        if self._has_data:
            self._has_data = False
            return True
    ...
    def server_entry(self):
        while not self.shutdown:
            ...
            nph = hk32['CreateNamedPipeA'](
                ...
                ctypes.c_uint(self._max_clients),
                ctypes.c_uint(self._max_message_size), ctypes.c_uint(self._max_message_size),
            ...
            client = ServerClient(nph, self.mode, self._max_message_size)
